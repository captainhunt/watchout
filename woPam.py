""" A library watchout uses to read/edit Linux-PAM configuration files.
...
def parseaccessconf(f='/etc/security/access.conf'):
  """Parses the watchout section in access.conf
...
if system() == 'Linux':
  ...
  def addUserLimits(username, bfrom, bto, f='/etc/security/time.conf'):
    ...
    """
    getconfsection(f) #Check if watchout section exists
    ...
    try:
      ...
      return False
    m = re.sub('(## WATCHOUT END)', line + '\\1', s)
    fn.write(m)
    return True

  def getconfsection(conffile):
    """Returns the content of the watchout section in a file (access.conf or time.conf)

    Also used to check if the watchout section is set correctly.
    Arguments: conffile (string)

    """
    s = open(conffile).read()
    check = re.compile('## WATCHOUT START|## WATCHOUT END').findall(s)
    # If the watchout section lines '## WATCHOUT START' or '## WATCHOUT END' are not
    # found, exit with an error.
    if not len(check):
      exit("Error: Could not find watchout section in '%s'" % conffile)
    elif len(check) != 2:
      exit("Error: Incorrect format of watchout section in '%s'" % conffile)
    # Otherwise, get and return the content between the section lines.
    m = re.compile('## WATCHOUT START\n(.*)## WATCHOUT END', re.S).findall(s)
    return m[0]

  def isUserLimited(user, f='/etc/security/time.conf'):
    ...
    x = re.compile('^\*;\*;' + user + ';Al0000-2400$', re.M).search(s)
    if x:
      return False
  ...
  def isUserLimitedNow(user, f='/etc/security/time.conf'):
    ...
    """
    if not isUserLimited(user):
      return False
    s = getconfsection(f)
    m = re.compile('^\*;\*;' + user + ';(.*)$', re.M).findall(s)
  ...
  def lockUser(u, f='/etc/security/access.conf'):
    ...
    watchout uses access.conf to lock/disable user accounts from logging in.
    Arguments: username
...
# W I N D O W S
  ...
  print("OS %s not supported." % system())